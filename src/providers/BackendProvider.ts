...
const fetchJson = async <T>(path: string, options?: ProviderFetchOptions): Promise<T> => {
...
const backendProvider: Provider<ProviderLeague, ProviderTeam, ProviderScoreCard> = {
  id: 'backend',
  name: 'OnlyScores API',
  async getLeagues(options?: ProviderFetchOptions) {
    const payload = await fetchJson<ProviderLeaguesResponse>(endpoints.leagues, options);
    return payload.leagues;
  },
  async getTeams(leagueId: string, options?: ProviderFetchOptions) {
    const query = buildQuery({ leagueId });
    const payload = await fetchJson<ProviderTeamsResponse>(`${endpoints.teams}${query}`, options);
  ...
  async getScores(request: ProviderScoresRequest, options?: ProviderFetchOptions) {
    ...
    const payload = await fetchJson<ProviderScoresResponse>(`${endpoints.scores}${query}`, options);
